...
fn check_hex_key_length(key: HexKey) -> Result<HexKey> {
    match hex::decode(&key).unwrap().len() {
        32 => Ok(key),
        _ => Err(AppError::Custom("Key length wrong!".to_string()))
    }
}
...
fn check_priv_key(key: HexKey) -> Result<HexKey> {
    check_key_is_valid_hex(key)
        .and_then(check_hex_key_length)
}

fn generate_priv_vk_from_priv_sk(priv_sk: HexKey) -> Result<HexKey> {
    check_priv_key(priv_sk)
...
println!("{:?}", priv_sk);
    ...
    fn should_not_panic_if_key_length_correct() {
        let key = generate_random_scalar_mod_order()
            .and_then(convert_scalar_to_hex_key)
            .unwrap();
        let _result = check_priv_key(key).unwrap();
    }

    #[test]
    #[should_panic]
    fn should_panic_if_key_length_too_long() {
        let key_long: HexKey = "fd4eef494e70a5d3b0309aa3ad0934dc07cc602731fd1b4b6a85702ddeeca00700".to_string();
    ...
    fn should_panic_if_key_length_too_short() {
        let key_short: HexKey = "fd4eef494e70a5d3b0309aa3ad0934dc07cc602731fd1b4b6a85702ddeeca0".to_string();
    ...
    fn should_panic_if_key_length_is_odd() {
        let invalid_hex: HexKey = "fd4eef494e70a5d3b0309aa3ad0934dc07cc602731fd1b4b6a85702ddeeca00".to_string();
    ...
    fn should_panic_if_key_not_valid_hex() {
        let invalid_hex: HexKey = "fd4eef494e70a5d3b0309aa3ad0934dc07cc602731fd1b4b6a85702ddeeca007xx".to_string();
        let _result = check_key_is_valid_hex(invalid_hex).unwrap();
    }

    #[test]
    fn should_generate_private_view_key_from_private_spend_key() {
        let priv_sk: HexKey = "fd4eef494e70a5d3b0309aa3ad0934dc07cc602731fd1b4b6a85702ddeeca007".to_string();
        let expected_key: HexKey = "d1546a8515e0488990db0f2d01666efc74bc26197c543047b766d8652db0b909".to_string();
